...
// engineConnRejectNoter is the optional shape of [wgengine.Engine]
// when the engine is built with connreject support. It is type-asserted
// rather than declared on the Engine interface so min builds do not
// pull in tailscale.com/net/connreject.
type engineConnRejectNoter interface {
...
func (e *extension) Init(host ipnext.Host) error {
	if e.sb != nil {
		...
		if eng, ok := e.sb.Sys().Engine.Get().(engineConnRejectNoter); ok {
