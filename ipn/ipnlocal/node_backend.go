...
func (nb *nodeBackend) PeerIsReachable(rp RouteCheckReport, p tailcfg.NodeView) bool {
	...
	if !self.HasCap(tailcfg.NodeAttrClientSideReachability) && !routecheckEnabled {
	...
	if !self.HasCap(tailcfg.NodeAttrClientSideReachabilityRouteCheck) && !routecheckEnabled {
