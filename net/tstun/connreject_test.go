...
// TestSetConnRejectNoteUninstall verifies that SetConnRejectNote(nil)
// safely clears the previously installed callback. The Wrapper's
// connRejectNote field is an atomic.Value (rather than a typed
// atomic.Pointer) to avoid pulling tailscale.com/net/connreject into
// min builds. Storing nil into atomic.Value panics, so the setter
// stores a typed-nil func; this test guards that path against
// regression and verifies notifyConnRejectTSMPSent is a safe no-op
// after uninstall.
func TestSetConnRejectNoteUninstall(t *testing.T) {
	t.Parallel()
	bus := eventbustest.NewBus(t)
	_, w := newFakeTUN(t.Logf, bus, false)
	defer w.Close()
