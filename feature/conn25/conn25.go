...
func (c *client) reserveAddresses(appName string, domain dnsname.FQDN, dst netip.Addr, ttlSecs uint32) (*addrs, error) {
	if !dst.IsValid() {
	...
	if err := c.assignments.insertFromTTL(as, ttlSecs); err != nil {
		return nil, err
	}
...
func (c *Conn25) mapDNSResponse(buf []byte) []byte {
	...
	for {
		...
		case dnsmessage.TypeA, dnsmessage.TypeAAAA:
			...
			answers = append(answers, dnsResponseRewrite{domain: queriedDomain, dst: dstAddr, ttlSecs: h.TTL})
		default:
...
func (c *client) rewriteDNSResponse(appName string, hdr dnsmessage.Header, questions []dnsmessage.Question, answers []dnsResponseRewrite) ([]byte, error) {
	...
	for _, rw := range answers {
		as, err := c.reserveAddresses(appName, rw.domain, rw.dst, rw.ttlSecs)
		if err != nil {
			return nil, err
		}
		if !as.isValid() {
