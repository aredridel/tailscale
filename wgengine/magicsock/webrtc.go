...
// isConnected reports whether the peer identified by its disco key has an
// active WebRTC data channel. It is used by the debug knob
// TS_DEBUG_ALWAYS_USE_WEBRTC to decide whether DERP data sends can be
// suppressed for a specific peer.
func (m *webrtcManager) isConnected(disco key.DiscoPublic) bool {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ps, ok := m.peerConnectionsByDisco[disco]
	return ok && ps.state == webrtcStateConnected && ps.dcRW.Load() != nil
}
...
func (m *webrtcManager) handleConnectionReady(event webrtcConnectionReadyEvent) {
	...
	// Set as bestAddr if better than current, or unconditionally when the
	// debug flag forces WebRTC.
	now := mono.Now()
	if debugAlwaysWebRTC() || betterAddr(webrtcAddr, event.ep.bestAddr) {
